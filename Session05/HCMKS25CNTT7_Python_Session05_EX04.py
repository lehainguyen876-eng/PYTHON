...
for i in range(1, branch_count + 1):
    ...
    # Thay classroom thành j cho đỡ trùng với i
    for j in range(1, 3): 
        while True:
            student_count = int(input(f"Nhap so hoc vien di hoc cua lop {j}: "))
            ...
            if student_count < 0:
                ...
                if student_count >= 20:
                    print(f"Chi nhanh {i} - Lop {j}: Lop hoc on dinh")
                else:
                    print(f"Chi nhanh {i} - Lop {j}: Lop can duoc nhac nho theo doi")
                break